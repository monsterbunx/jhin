...
# El SDK de Dart es glibc → no corre en Alpine (musl). El repo apt de Google es
# amd64-only; el SDK zip cubre amd64+arm64 y cualquier distro glibc.
if [ "$JHIN_OS" = "alpine" ]; then
  say "✗ dart: no soportado en Alpine (SDK glibc, sin build musl oficial)."
